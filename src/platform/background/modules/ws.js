(function initBackgroundWsModule(globalScope) {
  function createBackgroundWsModule({
    ...
    async function connectSendWs(cfg) {
      if (shouldDisableWsForHost(cfg?.api_base)) {
        if (!sendWsDisabledByHostLogged) {
          console.warn('[BG] send WS desactivado para host trycloudflare.com; se usa fallback por HTTP pull.');
      ...
      const wsTicket = await requestWsTicket(cfg, SEND_WS_TICKET_PATH, { from_account: fromAccount });
      if (!wsTicket) {
      ...
      if (sendWs && (sendWs.readyState === WebSocket.OPEN || sendWs.readyState === WebSocket.CONNECTING)) {
        return;
      }
      try {
        ...
        sendWs.onmessage = (ev) => {
          try {
            ...
            if (data && data.type === 'tasks' && Array.isArray(data.items) && data.items.length > 0) {
        ...
        sendWs.onclose = (ev) => {
          ...
          if (ev.code !== 1000 && state.isRunning) {
            ...
            if (sendWsFailedAttempts >= sendWsMaxRetries) {
              console.warn('[BG] WS send deshabilitado temporalmente tras errores de conexión. Se mantiene fallback por HTTP pull.');
              return;
            }
    ...
    function normalizeJobsWsEvent(raw) {
      ...
      const eventType = String(raw.type || raw.event_type || '').trim().toLowerCase();
      const eventId = String(raw.event_id || raw.id || '').trim() || null;
      const payload = raw.payload && typeof raw.payload === 'object' ? raw.payload : raw;
      if (eventType === 'jobs_updated' || eventType === 'jobs.updated' || eventType === 'job.updated') {
        return {
          type: 'jobs_updated',
          eventId,
          payload,
    ...
    function connectJobsWs(cfg) {
      ...
      if (shouldDisableWsForHost(cfg.api_base)) {
        if (!jobsWsDisabledByHostLogged) {
          console.warn('[BG] jobs WS desactivado para host trycloudflare.com; se usa fallback por polling.');
      ...
      requestWsTicket(cfg, JOBS_WS_TICKET_PATH, {}).then((wsTicket) => {
        ...
        if (!url || (jobsWs && (jobsWs.readyState === WebSocket.OPEN || jobsWs.readyState === WebSocket.CONNECTING))) return;
        try {
          ...
          jobsWs.onmessage = (ev) => {
            try {
              ...
              chrome.runtime.sendMessage({ type: event.type, payload: event.payload, event_id: event.eventId }).catch(() => {});
            } catch (_) {}
          };
          jobsWs.onclose = (ev) => {
            jobsWs = null;
            ...
            if (ev.code !== 1000) {
              ...
              console.warn('[BG] jobs WS reintentando en', Math.round(jobsWsReconnectDelayMs / 1000), 's...');
              setTimeout(() => {
                authModule.loadSettings().then((c) => {
          ...
          jobsWs.onerror = (e) => {
            console.warn('[BG] jobs WS onerror (detalle del navegador puede ser limitado):', e?.message || e);
            jobsWs = null;
          };
        } catch (e) {
