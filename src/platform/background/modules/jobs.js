      ...
      try {
        ...
        if (!result.success && errorCode === "thread_identity_not_verified") {
          ...
          chrome.runtime
            ...
            .catch((e) => {
              if (!isPopupClosedError(e)) {
                console.warn(
                  "[BG] Failed to notify popup (thread_identity_soft):",
                  e?.message || e
                );
              }
            });
        ...
        if (result.success) {
          ...
          if (dryRun) {
            ...
            state.nextDMTime =
              Date.now() + randomBetween(config.minDelayBetweenDMs, config.maxDelayBetweenDMs);
          }
        } else {
