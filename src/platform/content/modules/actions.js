(function initContentActionsModule(globalScope) {
  function createContentActionsModule({ selectorsModule, observersModule }) {
    const { HUMAN_CONFIG, randomBetween, sleep, isElementVisible, humanDelay, waitForElement, waitForSendButton } = selectorsModule;
    const { normalizeIgUsername, isLikelyIgUsername, waitForThreadOpened, waitForThreadIdentity, waitForDocumentComplete } = observersModule;
    ...
    async function insertTextContentEditable(editable, text) {
      ...
      const target = editable.tagName === 'P' ? editable.closest('[contenteditable="true"]') || editable : editable;
      target.textContent = '';
      await sleep(50);
      target.textContent = text;
      target.dispatchEvent(new InputEvent('beforeinput', { bubbles: true, inputType: 'insertFromPaste', data: text }));
    ...
    async function typeHumanLike(element, text) {
      ...
      if (isInputOrTextarea) {
        ...
        await humanDelay(300, 600);
        element.value = '';
        element.dispatchEvent(new InputEvent('beforeinput', { bubbles: true, inputType: 'deleteContent' }));
        ...
        for (let i = 0; i < text.length; i++) {
          ...
          if (Math.random() < 0.05) await humanDelay(HUMAN_CONFIG.thinkingPauseMin, HUMAN_CONFIG.thinkingPauseMax);
        }
      } else {
    ...
    async function navigateToProfile(username) {
      ...
      if (window.location.href.includes(`/${username}/`) || window.location.href.includes(`/${username}`)) {
    ...
    async function navigateToDirect() {
      ...
      if (!ok) {
        console.warn('[BeLeadAI] Timeout navegando a /direct; seguimos fuera de /direct:', window.location.pathname);
      }
      return ok;
    }
    ...
    async function directSearchAndOpenThread(username) {
      ...
      console.log('[BeLeadAI] Input de búsqueda encontrado:', searchInput.placeholder || searchInput.getAttribute('aria-label') || 'sin placeholder');
      searchInput.focus();
      await humanDelay(300, 600);
      searchInput.value = '';
      ...
      function findDropdownScopeByHeading() {
        ...
        for (const h of headings) {
          ...
          let scope = h.closest('div[style*="--x-minHeight"]') || h.closest('div[role="dialog"]') || h.closest('section') || h.parentElement;
      ...
      function rowStrongMatch(row, u) {
        ...
        const usernameBoundaryRegex = new RegExp(`(^|[^a-z0-9._])@?${escapedUsername}($|[^a-z0-9._])`, 'i');
      ...
      while (Date.now() - startTs < timeoutMs) {
        ...
        if (resultRows.length > 0 && !sawVisibleRows) {
          ...
          resultRows.forEach((row, idx) => {
            ...
            console.log(`[BeLeadAI] Fila ${idx + 1}: textUsers=[${textUsers.join(', ')}], hrefUsers=[${hrefUsers.join(', ')}]`);
          });
        }
        ...
        if (clickPlan.length === 0 && resultRows.length === 1 && elapsed > 8000) {
          if (rowHasExtraIdentitySignal(resultRows[0], uname) || rowHasTextExactUsername(resultRows[0], uname)) {
            clickPlan.push({ priority: 2, match: { row: resultRows[0], reason: 'single-visible-row', href: null, confidence: 'soft' } });
          } else {
            console.warn('[BeLeadAI] Se descarta single-visible-row sin señal extra de identidad para @' + uname);
          }
        }
        ...
        for (const item of clickPlan) {
          ...
          if (lastDirectThreadCandidateUsername) {
            console.log('[BeLeadAI] Username candidato desde row:', lastDirectThreadCandidateUsername);
          }
        ...
        if (!clickedAny) {
          if (resultRows.length > 0) {
            if (Date.now() - lastDebugLogTs > 1200) {
              const sample = (resultRows[0].innerText || resultRows[0].textContent || '').replace(/\s+/g, ' ').trim().slice(0, 90);
    ...
    function hasRecentMessageEcho(expectedSnippet) {
      ...
      const messageNodes = document.querySelectorAll('main div[role="listitem"] span[dir="auto"], main div[role="listitem"] div[dir="auto"]');
    ...
    async function sendDMViaDirect(username, message, dryRun) {
      ...
      try {
        ...
        let identityOk = await waitForThreadIdentity(username, 7000, lastDirectThreadCandidateUsername, lastDirectThreadCandidateConfidence);
        if (!identityOk) {
          await sleep(1200);
          identityOk = await waitForThreadIdentity(username, 4500, lastDirectThreadCandidateUsername, lastDirectThreadCandidateConfidence);
        }
        if (!identityOk) {
        ...
        const editableEl = messageInput.tagName === 'P' || messageInput.getAttribute('contenteditable') !== 'true'
          ? (messageInput.closest('[contenteditable="true"]') || document.querySelector('div[contenteditable="true"]'))
          : messageInput;
        if (editableEl) {
          editableEl.focus();
          editableEl.textContent = '';
          editableEl.dispatchEvent(new InputEvent('input', { bubbles: true, inputType: 'deleteContent' }));
        ...
        const writtenText = ((editableEl || messageInput).textContent || (editableEl || messageInput).value || '').replace(/\s+/g, ' ').trim();
        const expectedPrefix = message.replace(/\s+/g, ' ').trim().substring(0, Math.min(30, message.length));
        ...
        const sendConfirmation = await waitForSendConfirmation(messageInput, editableEl, message, 7000);
      ...
      console.log('[BeLeadAI] ========== sendDMViaDirect FIN ========== success:', result.success, 'error:', result.error);
      return result;
    }
    ...
    async function clickMessageButton() {
      ...
      if (!msgBtn) {
        console.error('[BeLeadAI] ERROR: No se encontró el botón de mensaje. Instagram puede haber cambiado el DOM.');
        return false;
      }
    ...
    async function typeAndSendMessage(message) {
      ...
      const sendBtn = await waitForSendButton(5000);
      if (!sendBtn) {
        console.error('[BeLeadAI] ERROR: No se encontró el botón de enviar (probado svg[aria-label=Send] y selectores clásicos).');
        return false;
      }
    ...
    async function sendDM(username, message, dryRun = true) {
      ...
      if (!isLikelyIgUsername(normalizedUsername)) {
        return { success: false, username: null, error: 'invalid_username', dryRun, steps: ['validate_username'] };
      }
      ...
      if (isInstagramHost) {
        ...
        console.log('[BeLeadAI] ========== sendDM FIN ========== success:', directResult.success, 'error:', directResult.error, 'steps:', directResult.steps);
        return directResult;
      }
      ...
      console.log('[BeLeadAI] ========== sendDM FIN ========== success:', result.success, 'error:', result.error, 'steps:', result.steps);
      return result;
    }
