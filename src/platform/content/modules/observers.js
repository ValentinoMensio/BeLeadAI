(function initContentObserversModule(globalScope) {
  function createContentObserversModule({ selectorsModule }) {
    ...
    function isLikelyIgUsername(v) {
      ...
      if (["direct", "accounts", "explore", "reels", "stories", "message", "mensaje"].includes(token)) return false;
      return true;
    }
    ...
    function collectThreadIdentityCandidates() {
      ...
      const textNodes = document.querySelectorAll('header h1, header h2, header a, header span, header div');
    ...
    async function waitForThreadIdentity(expectedUsername, timeoutMs = 5000, selectedRowUsername = null, selectedRowConfidence = null) {
      const expected = normalizeIgUsername(expectedUsername);
      if (!expected) return false;
      ...
      while (Date.now() - start < timeoutMs) {
        ...
        if (allowSelectedRowFallback) {
          ...
          if (selectedRowNoConflictStreak >= 2) {
            console.warn('[BeLeadAI] Thread verificado por row fuerte + URL thread (sin conflicto visible estable) para @' + expected);
            return true;
          }
        } else {
    ...
    function waitForDocumentComplete(timeoutMs = 15000, requireDirectPath = false, intervalMs = 100) {
      return new Promise((resolve) => {
        const timeout = setTimeout(() => {
          ...
          const inDirect = !!(window.location.pathname && window.location.pathname.startsWith('/direct'));
          resolve(inDirect);
        }, timeoutMs);
        ...
        const check = setInterval(() => {
          ...
          if (!ready) return;
          if (requireDirectPath && !(window.location.pathname && window.location.pathname.startsWith('/direct'))) return;
