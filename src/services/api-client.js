...
function isUpdateRequiredError(status, data) {
  const code = String(data?.error?.code || "").trim().toUpperCase();
  return Number(status || 0) === 426 || code === "CLIENT_UPDATE_REQUIRED";
}

function persistVersionBlockState(status, data) {
  if (!chrome?.storage?.local) return;
  const details = data?.error?.details && typeof data.error.details === "object" ? data.error.details : {};
  const payload = {
...
function buildApiError(status, data, fallbackMessage = "Error de la API.", resp = null) {
  ...
  const details = data?.error?.details && typeof data.error.details === "object" ? data.error.details : null;
  const traceId =
...
export async function apiFetch(baseUrl, path, options = {}) {
  ...
  if (!headers.Authorization) {
    const error = buildApiError(401, { error: { code: "AUTH_REQUIRED" } }, "Faltan credenciales. Configurá en Opciones.");
  ...
  const to = timeoutMs ? setTimeout(() => controller.abort(), timeoutMs) : null;
    const doFetch = async (headersToUse) => fetch(url, {
      method,
  ...
  try {
    ...
    if (!resp.ok) {
      ...
      const retrySec = typeof window.retryAfterFromResponse === "function"
        ? window.retryAfterFromResponse(resp, data)
        : null;
      const errorMessage = typeof window.formatApiErrorForUser === "function"
    ...
    if (e?.name === "AbortError") {
      const error = buildApiError(0, { error: { code: "REQUEST_TIMEOUT" } }, "La solicitud tardó demasiado.");
      return { ok: false, status: 0, errorMessage: error.message, error };
    }
...
export function classifyFetchError(e) {
  ...
  if (msg.includes("certificate") || msg.includes("ssl") || msg.includes("tls") || msg.includes("https") || msg.includes("secure")) {
    return { kind: "tls", message: "Error de certificado/HTTPS. Probá con https:// y certificado válido." };
  }
...
async function probeLoginFailure(cfg) {
  ...
  if (!apiKey || !apiBase || !isSecureApiBase(apiBase)) {
    return { errorCode: null, errorDetails: null, planName: null, apiErrorMessage: null, status: 0 };
  }
  try {
    ...
    const errorDetails = data?.error?.details && typeof data.error.details === "object" ? data.error.details : null;
    const planName =
...
export async function fetchPing(cfg) {
  ...
  if (!isSecureApiBase(base)) {
    const error = buildApiError(0, { error: { code: "HTTPS_REQUIRED" } }, "La URL base debe usar HTTPS.");
  ...
  try {
    ...
    const errorDetails = parsed?.error?.details && typeof parsed.error.details === "object" ? parsed.error.details : null;
    ...
    const error = buildApiError(0, { error: { code: "NETWORK_ERROR" } }, message || "Error de red.");
    return { ...empty, errorMessage: error.message, error };
  }
}
